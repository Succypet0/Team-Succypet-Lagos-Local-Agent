...
with tab1:
    ...
    if st.button("Generate Review"):
        with st.spinner("Agent is reasoning... (usuallly less than 60)"):
            payload = {"persona": persona_a, "product_details": product}
            
            response = requests.post("http://n8n:5678/webhook/test-a-simulate", json=payload, timeout=60)
            ...
            if response.status_code == 200:
                ...
                st.error(f"Backend orchestration failed. Status Code: {response.status_code}")
...
with tab2:
    ...
    if st.button("Get Recommendation"):
        with st.spinner("Analyzing constraints, reasoning and ranking items... (usually less than 60s)"):
            payload = {"persona": persona_b}
            
            response = requests.post("http://n8n:5678/webhook/task-b-review", json=payload,timeout = 60)
